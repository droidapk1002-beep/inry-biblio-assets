...
function renderAdminDocTable() {
  ...
  html += `<tr id="bulk-edit-panel" style="display:none;"><td colspan="8" style="padding:0.75rem 1rem;background:var(--surface-alt,#f5f0e8);border-bottom:2px solid var(--kraft-line);">
    <div style="display:flex;gap:var(--space-3);align-items:flex-end;flex-wrap:wrap;">
      ...
      <label style="font-size:var(--fs-xs);"><input type="checkbox" id="bulk-chk-host"> Hébergeur
        <select id="bulk-val-host" disabled style="display:block;margin-top:2px;"></select></label>
      ...
      <label style="font-size:var(--fs-xs);"><input type="checkbox" id="bulk-chk-year"> Année
        <input type="number" id="bulk-val-year" disabled placeholder="2025" min="2000" max="2099" style="display:block;margin-top:2px;width:90px;"></label>
  ...
  if (bulkEditBtn && bulkPanel) {
    ...
    if (bulkHosts) bulkHosts.innerHTML = '<option value="">— choisir —</option><option value="drive">Google Drive</option><option value="mega">Mega</option><option value="other">Autre</option>';
    ...
    ['bulk-chk-level','bulk-chk-stream','bulk-chk-subject','bulk-chk-type','bulk-chk-host','bulk-chk-wilaya','bulk-chk-trimestre','bulk-chk-year'].forEach(chkId => {
      const chk = document.getElementById(chkId);
      if (chk) chk.addEventListener('change', () => {
        const ctrl = chk.parentElement.querySelector('select, input[type="number"]');
    ...
    document.getElementById('bulk-apply-btn')?.addEventListener('click', () => {
      ...
      if (document.getElementById('bulk-chk-host')?.checked) fields.host = bulkHosts.value;
